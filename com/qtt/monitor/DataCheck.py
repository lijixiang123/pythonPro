...
#机器人
def robot_alert(content):
    url_alert = "https://qyapi.weixin.qq.com/cgi-bin/webhook/send?key=acf9685b-b208-48fb-aadd-89176cbd4f88"
    headers={
        'Content-Type':'application/json'
    }
    body = {
...
#机器人传如参数
def robot_alertbyparm(content,key):
...
def get_data(v_sql,engine):
    ...
    if errCode==1000:
        print("presto返回错误码1000，尝试5次重试查询")
...
def check_data(data,cols):
    ...
    for row_data in datas:
        ...
        while i < len(row_data):
            ...
            print("列名_"+str(columns[i])+":"+str(row_data[i]))
...
def get_uids(users,alert_type):
    ...
    for name in us:
        ...
        if len(col_name) > 0 and name.count("(") == 0 and name.count(")")==0 and name.count("|") ==0 and name.count("-") ==0:
...
if __name__ == "__main__":
    #print(get_user('hanzhengding'))
    alert_msg('5237','2','hello zhengding','哈喽正鼎')