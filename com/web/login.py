...
def post_login():
    ...
    # 获取并验证客户端提交的参数
    ##############################################################
    username = requests.get_form('username', '帐号')
    password = requests.get_form('password', '密码')
    verify = requests.get_form('verify', '验证码')
    ip = requests.get_ip()

    ##############################################################
    # 从session中读取验证码信息
    ##############################################################
    s = requests.get_session()
    ...
    if verify.upper() != verify_code:
        return requests.return_msg(-1, '验证码错误')
